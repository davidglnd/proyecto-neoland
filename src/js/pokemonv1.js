...
function encontrarPokemon() {
    ...
    for (let i = 0; i < pokedex.length; i++) {
        let stringPokedexId = String(pokedex[i].id)//declaro la variable en string para poder hacer el if con el id ===
        if (pokedex[i].name.english.toLowerCase() === campoBusqueda.value || stringPokedexId === campoBusqueda.value || pokedex[i].name.english === campoBusqueda.value ){//este if verifica si el nombre o el id esta dentro de la pokedex
    ...
    if(idPokemon === undefined){
        ...
        crearNuevaLista.appendChild(mensajeError)
    }
}
